...
def run_game(bot_list):
    ...
    while running:
        if stop_count >= 3 or len(free_coordinates) == 0: # game ends without valid move
            scores = score(board)
            res["scores"] = scores
            break
        ...
        if bot_move and isinstance(bot_move, tuple) and bot_move in free_coordinates: # If valid move
